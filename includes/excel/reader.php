...
//end of changes by mmp		
    ...
    /*
     * Added 5-22-07 Ethan Wilder
     *
     * This function trims the # of columns and rows down to the maximum needed. 
     * Before this function the reader object would have too many blank columns and rows
     * this was annoying so I wrote this. 
     *
     * @param $sheet - the spreadsheet to parse since this object can contain multiple sheets
     */
    function trimSheet($sheet) {

	$this->_trimCols($sheet);
	$this->_trimRows($sheet);

    }

    function _trimCols($sheet) {

	/* Column Loop */
	for($i=$this->sheets[$sheet]['numCols']; $i>0; $i--) {

		for($j=0; $j < $this->sheets[$sheet]['numRows']; $j++) {

			if($this->sheets[$sheet]['cells'][$j][$i] != "") {
				$this->sheets[$sheet]['numCols'] = $i;
				return;
			}
		}
	}
	...
	/* Row Loop */
	for($i=$this->sheets[$sheet]['numRows']; $i>0; $i--) {

		for($j=0; $j < $this->sheets[$sheet]['numCols']; $j++) {

			if($this->sheets[$sheet]['cells'][$i][$j] != "") {
				$this->sheets[$sheet]['numRows'] = $i;
				return;
			}
		}
	}
