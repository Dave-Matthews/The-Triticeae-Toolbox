...
if (($id = array_search("Update", $_POST)) != null) {
    ...
    updateTable($_POST, "experiment_set", array("experiment_set_uid"=>$id));
}

$searchstring = '';
if(isset($_REQUEST['search']) && $_REQUEST['search'] != "") {
  $tablesToSearch = array("experiment_set");
  $found = array();
...
<div id="primaryContentContainer">
  <div id="primaryContent">
	...
	<form action="<?php echo $config['base_url']; ?>login/edit_experiments.php" method="post">
...
function editTrialRow($where, $page, $start="0") {
  $ignore = array("experiment_uid");
  editGeneral("experiment_set", $where, $page, $ignore, "20", $start);
}
...
function editRangeTrials($minID, $maxID, $page) {
	$where = "experiment_set_uid < '$maxID' AND experiment_set_uid > '$minID'";
...
function editSelectTrials($IDRange, $page, $start) {
  if(is_array($IDRange)) {
    ...
    for($i=0; $i<count($IDRange); $i++) {
      if($i != 0) 
	$where .= " OR ";
      $where .= "experiment_set_uid = '$IDRange[$i]'";
    }
