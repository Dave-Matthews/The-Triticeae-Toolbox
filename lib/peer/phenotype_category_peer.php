 ...
 * Contains methods for extracting rows from the table 'phenotype_category'
 */
class phenotype_category_peer
{
  # IMPORTANT: Do not modify this file
  /* begin-auto-gen */
    protected static $base_sql = 'select phenotype_category_uid, phenotype_category_name, updated_on, created_on from phenotype_category';
  ...
  public static function get_all() {
    ...
    while ($row = mysql_fetch_assoc($query)) {
      ...
      $results[] =& new $modelname($row['phenotype_category_uid'], $row['phenotype_category_name'], $row['updated_on'], $row['created_on']);
    }
    return $results;
  }
  ...
  public static function get_by_phenotype_category_uid($arg0) {
    ...
    $modelname = substr(__CLASS__, 0, -5);
    $temp =& new $modelname($row['phenotype_category_uid'], $row['phenotype_category_name'], $row['updated_on'], $row['created_on']);
    return $temp;
  }
  ...
  # auto-generated function
  public static function get_by_phenotype_category_name($arg0) {
    $sql = self::$base_sql.' where phenotype_category_name = \''.$arg0.'\' limit 1';
    $query = mysql_query($sql);
    if (mysql_num_rows($query) <= 0) return null;
    $row = mysql_fetch_assoc($query);
    $modelname = substr(__CLASS__, 0, -5);
  ...
  # auto-generated function
  public static function get_by_phenotype_category_name_array(array $arg0) {
    if (empty($arg0)) return null;
    $sql = self::$base_sql.' where phenotype_category_name in ('.implode(',', $arg0).')';
    $query = mysql_query($sql);
    $results = null;
    while ($row = mysql_fetch_assoc($query)) {
      $modelname = substr(__CLASS__, 0, -5);
