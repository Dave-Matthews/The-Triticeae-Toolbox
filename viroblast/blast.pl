...
system("sed 's/<[^>]*>//g' $dataPath/$jobid.out > $dataPath/$jobid.pbrin");
if ( $? == -1 ) {
  print LOG "commandfailed: $!\n";
}
system("perl parseBlastReport.pl -i $dataPath/$jobid.pbrin -v -o $dataPath/pbr_$jobid.csv");
if ( $? == -1 ) {
  print LOG "commandfailed: parseBlastReport.pl $!\n";
}
system("perl addHspAnker.pl $jobid");
if ( $? == -1 ) {
  print LOG "commandfailed: addHspAnker.pl $!\n";
}
system("perl resultgraph.pl $jobid");
if ( $? == -1 ) {
  print LOG "commandfailed: resultgraph.pl $!\n";
}
