...
if(!$opt || $opt == 'wait') {
	$progressdot = "viroblast/image/progressdot.png";
...
if(!$opt || $opt == 'wait') {
          echo "<META HTTP-EQUIV=\"refresh\" content=\"3; URL=$config[base_url]viroblast/blastresult.php?jobid=$jobid&alignmentView=$alignmentView&opt=wait&dot=$dot\">\n";
...
if (file_exists($errFile) && filesize($errFile) > 0) {
	if(!$opt || $opt == 'wait') {
                ...
                echo "location.replace('$config[base_url]viroblast/blastresult.php?jobid=$jobid&opt=none')";
                echo "</script>";
	}else {
