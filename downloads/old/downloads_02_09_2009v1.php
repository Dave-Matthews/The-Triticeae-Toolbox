...
$cfg_file_rel_path = 'downloads/downloads_02_09_2009.php';
...
class Downloads
{
	...
	private function type1()
	{
		...
		echo "<p><em>Select multiple files by holding down the Ctrl key while selecting.
		Please limit your selection to a single breeding program for genotype file downloads.</em></p>";
	...
	private function type1_breeding_programs_year()
	{
		...
		<script type="text/javascript">
			...
			function load_experiments()
			{
                ...
                document.title='Loading Experiments...';
				new Ajax.Updater(
                    $('experiments_loader'),
			...
			function load_markers(o, d, mm, mmaf) {
                ...
                new Ajax.Updater($('markers_loader'), '<?php echo $_SERVER['PHP_SELF'] ?>?function=type1markers&bp='+ breeding_programs_str+'&yrs='+ years_str+'&exps='+experiments_str+'&o='+o+'&d='+d+'&m='+selmarkerids.join(',')+'&mm='+mm+'&mmaf='+mmaf,
		...
		// should this be phenotype experiments only? Yes
            ...
            $sql = <<< SQL
                ...
                FROM
                    markers m INNER JOIN (allele_frequencies a, marker_synonyms ms, markers_in_maps mim, map,
						datasets ds, datasets_experiments de, experiments e, experiment_types et)
               ...
               WHERE
                    e.experiment_type_uid = et.experiment_type_uid
						...
						AND e.experiment_uid = de.experiment_uid
						AND de.datasets_uid = ds.datasets_uid					
						AND ds.breeding_year IN ($years)
						AND ds.CAPdata_programs_uid IN ($CAPdata_programs)
...
SQL;
		...
		$res = mysql_query($sql) or die(mysql_error());
			...
			$fb($temp, 'number of rows in query');
            $markerids = array();
			if (mysql_num_rows($res) >= 1) {
                ?>
                <style type="text/css">
