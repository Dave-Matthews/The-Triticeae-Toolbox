...
$cfg_file_rel_path = 'downloads/downloads_02_25_2009.php';
...
require_once('../pear/File_Archive/Archive.php');
...
<?php
	...
	private function type1_markers()
	{
       ...
       // $firephp->log($min_maf," maf");
		//$firephp->log($max_missing," max missing");
		 ?><div class="notice"><p style="font-style: italic">Monomorphic markers will <u>NOT</u> be included in the QTLMiner/Tassel downloads.</p></div><?php
			...
			if (mysql_num_rows($res) >= 1) {
                ?><div style="padding-left: 20px">
					...
					<p style="font-style: italic"><?php echo ($num_removed) ?> markers will be removed</p>
					...
					<br />
                    <br />
                    <input type="button" value="Download for Tassel" onclick="javascript:getdownload_tassel();return false;" />
                    <input type="button" value="Download for QTLMiner" onclick="javascript:getdownload_qtlminer();return false;" />
                </div>
				<?php
			} else {
	...
	function type1_build_tassel()
	{
		...
		$dir = 'temp/';
		$filename = 'download_tassel'.chr(rand(65,80)).chr(rand(65,80)).chr(rand(64,80)).'.zip';
