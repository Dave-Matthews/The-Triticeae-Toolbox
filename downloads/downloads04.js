...
/*used when updating radio button for line selection (session, all, combine) */
    ...
    if (select1_str == "Phenotypes") {
		...
		function update_years(options) {
           ...
           function create_file(version) {
                ...
                var url=php_self + "?function=download_session_" + version + "&typeG=" + typeG[0].checked + "&typeGE=" + typeG[1].checked;
	...
	function use_session(version) {
	        var mm = $('mm').getValue();
                ...
                document.getElementById('step6').innerHTML = "";
                var url=php_self + "?function=step5lines&pi=" + phenotype_items_str + '&yrs=' + years_str + '&exps=' + experiments_str + '&mm=' + mm + '&mmaf=' + mmaf + '&mml=' + mml + '&use_line=yes' + "&typeGE=" + typeG[1].checked;
                document.title='Loading Markers...';
                var tmp = new Ajax.Updater($('step5'), url, {
                    onComplete: function() {
                         $('step5').show();
        ...
        function load_pheno_lines() {
			...
			function load_markers_lines( mm, mmaf, use_line) {
                ...
                var url=php_self + "?function=step5lines&pi=" + phenotype_items_str + '&yrs=' + years_str + '&exps=' + experiments_str + '&mm=' + mm + '&mmaf=' + mmaf + '&use_line=' + use_line + "&typeGE=" + typeG[1].checked;
                document.title='Loading Markers...';
            ...
            function select_download() {
                ...
                var typeG=document.getElementsByName('typeG');
                document.getElementById('step6').innerHTML = "";
                var url=php_self + "?function=verifyLines&typeP=" + typeP.checked + "&typeG=" + typeG[0].checked + "&typeGE=" + typeG[1].checked;
                var tmp = new Ajax.Updater($('step5'), url, {
                    onSuccess: function() {
                    if (typeG[0].checked) {
