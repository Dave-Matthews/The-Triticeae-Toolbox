...
function use_session(version) {
    ...
    mmaf = $('mmaf').getValue();
    mml = $('mml').getValue();
    typeG=document.getElementById('typeG');
    ...
    jQuery.ajax({
      type: "GET",
      url: php_self,
      data: "function=step5lines&pi=" + phenotype_items_str + "&exps=" + experiments_str + "&mm=" + mm + "&mmaf=" + mmaf + "&mml=" + mml + "&use_line=yes&typeGE=" + typeGE.checked + "&typeG=" + typeG.checked,
      success: function(data, textStatus) {
        jQuery("#step5").html(data);
        $('step5').show();
