...
define("SOURCEFILE", "OR06Corvallis_mn");		// the name of the meanse file without the file extension (must be in this directory)
...
define("COL_LODGING", 16);
define("COL_PLUMPGRAIN", 17);
define("COL_TESTWEIGHT", 18);
...
for($i = ROW_START; $i <= ROW_STOP; $i++)
{
	 ...
	 */
	$sql = "select line_record_uid from line_records where line_record_name = '$line_name' limit 1";
	$res = mysql_query($sql) or die(mysql_error() . "<br>$sql");
	if (1 == mysql_num_rows($res)) // the specified line record already exists in the database
	{
		$line_record = mysql_fetch_assoc($res);
		$line_record_uid = $line_record['line_record_uid'];
	}
	else
	{
