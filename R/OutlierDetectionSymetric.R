...
#Libraries
##Package "lsmeans" requires pre-loaded packages: "estimability", "mvtnorm", "multcomp", "sandwich", "zoo", "TH.data", "coda", "xtable"
...
BonfHolmOutlier <- function(resi){
	CalcMedian <- median(resi, na.rm=TRUE)
	AbsDev <- abs(resi - CalcMedian)
	#Calculate left and right MAD values
	left.MAD <- median(AbsDev[resi<=CalcMedian], na.rm=TRUE)*1.4826
	right.MAD <- median(AbsDev[resi>=CalcMedian], na.rm=TRUE)*1.4826
	
if(OK[1,i] == TRUE){
	#MAD standardized residuals. Here including the values at the exact median in the "left.MAD". they need to be on one side or the other.  	It really will not matter which side they are on because values on the median will not be outliers.
	res_MAD <-ifelse((resi<=CalcMedian), (resi/left.MAD), (resi/right.MAD))
	...
	#Create vectors/matrices out of the list of BH tests
	bholm <- cbind(res2[[1]][,2])
	index <- cbind(res2[[2]])
	
	#Cancel the built in sort function from mt.rawp2adjp
...
for (i in 1:c) {
	...
	AbsDev2 <- abs(resi[,i] - m)
	left.MAD2 <- median(AbsDev2[resi[,i]<=m], na.rm=TRUE)
	right.MAD2 <- median(AbsDev2[resi[,i]>=m], na.rm=TRUE)
	test <- (left.MAD2 >= 0.4 & right.MAD2 >= 0.4)
