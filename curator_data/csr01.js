...
function update_zoom(frm) {
  ...
  win = new Ajax.Updater($('step2'), url, {method: 'post', postBody: param}, {
        onComplete : function() {
            $('step2').show();
...
function display2(option) {
  ...
  var url = php_self + "?function=display&uid=" + uid;
  win = new Ajax.Updater($('step2'), url, {
        onComplete : function() {
            $('step2').show();
            document.title = title;
        }
...
function save_session() {
	...
	win = new Ajax.Updater($('status'), url, {asynchronous:false}, {
        onComplete : function() {
            $('status').show();
            document.title = title;
        }
	...
	url = "side_menu.php";
	win = new Ajax.Updater($('quicklinks'), url, {
        onComplete : function() {
            $('quicklinks').show();
            document.title = title;
        }
...
function update_DateTime() {
	...
	win = new Ajax.Updater($('status'), url, {
	      onComplete : function() {
	          $('status').show();
	          document.title = title;
	    }
	...
	win = new Ajax.Updater($('download'), url, {
	      onComplete : function() {
	          $('download').show();
	          document.title = title;
	    }
...
function update_f1() {
  ...
  if (formula1 == "NWI1") {
    ...
    w2 = 670;
    w1 = 550;
    formula2 = "3*((W3-W2) - 0.2*(W3-W1)*(W3/W2))";
