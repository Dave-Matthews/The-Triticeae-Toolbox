...
// 02/23/2011 JLee  Put CAP entry code values in synonyms table 
		...
		if (strpos($uploadfile, ".xls") === FALSE) {
            ...
            */
            ...
            // First, locate first line of data in the file
                ...
                foreach ($header as $columnOffset => $columnName) { 
	...
	private function type_Database()  {
		...
		for ($irow = $firstline+1; $irow <=$rows; $irow++)  {
			...
			if ($line_uid === FALSE) {
				...
				$rlinsyn=mysql_query($sql) or die("Line insertion failed - ". mysql_error());
				...
				$sql = $sql_beg.$sql_mid.$sql_end;
				$rlinsyn=mysql_query($sql) or die("Line update failed - " . mysql_error());
						
			} else {
				$line_uids = implode(",",$line_uid);
				error(0, "$line is found in multiple records($line_uids), in line record table, please fix");
			}
			//echo "CAP code - ". $CAPcode . "<br>";
            if (!empty($CAPcode)){
				if ($CAPcode == $line) {
					...
					if ($linesyn_uid === FALSE) {
                        ...
                        $sql = "INSERT INTO line_synonyms (line_record_uid,line_synonym_name, updated_on, created_on)
							VALUES($line_num, '$CAPcode', NOW(),NOW())";
                        //echo "Synonyms insert sql - " . $sql  . "<br>";           
						mysql_query($sql) or die("Line synonyms insert failed - ". mysql_error());
        ...
        $lin_table=mysql_query($sql) or die("Input log entry creation failed - " . mysql_error());
		$footer_div = 1;
        include($config['root_dir'].'theme/footer.php');
